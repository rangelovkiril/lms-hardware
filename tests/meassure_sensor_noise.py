...
def test_measurement_noise(
    ...
    for i in range(num_samples):
        ...
        if detected and station.distance > 0:
            ...
            if (i + 1) % 20 == 0:
                log("INFO", "NOISE TEST", f"Progress: {i + 1}/{num_samples} samples (valid: {valid_count})")
        else:
            invalid_count += 1
    ...
    log("INFO", "NOISE TEST", f"LIDAR1 - Mean: {np.mean(lidar1_readings):.4f}m, Std: {np.std(lidar1_readings):.4f}m")
    log("INFO", "NOISE TEST", f"LIDAR2 - Mean: {np.mean(lidar2_readings):.4f}m, Std: {np.std(lidar2_readings):.4f}m")
    log("INFO", "NOISE TEST", f"Detection rate: {valid_count}/{num_samples} ({valid_count/num_samples*100:.1f}%)")
    log("INFO", "NOISE TEST", "=" * 60)
    ...
    if len(distances) < 10:
        ...
        log("ERROR", "NOISE TEST", f"  4. Distance threshold is {station.dist_threshold}m - object might be beyond this")
        station.disable()
        return
    ...
    log("INFO", "NOISE TEST", f"RECOMMENDED measurement_noise_std: {recommended_noise:.4f}")
    log("INFO", "NOISE TEST", "=" * 60)
    
    if std_dist > 0:
        log("INFO", "NOISE TEST", "\nDistance distribution:")
        log("INFO", "NOISE TEST", f"  Within 1σ: {np.sum(np.abs(distances - mean_dist) <= std_dist) / len(distances) * 100:.1f}%")
        log("INFO", "NOISE TEST", f"  Within 2σ: {np.sum(np.abs(distances - mean_dist) <= 2*std_dist) / len(distances) * 100:.1f}%")
        log("INFO", "NOISE TEST", f"  Within 3σ: {np.sum(np.abs(distances - mean_dist) <= 3*std_dist) / len(distances) * 100:.1f}%")
