...
def test_process_noise(duration: float = 10.0, sample_interval: float = 0.05):
    """
    ...
    while (time.time() - start_time) < duration:
        ...
        if detected and station.distance > 0:
            pos = spherical_to_cartesian(
                station.distance, station.azimuth, station.elevation
            )
            ...
            if len(positions) % 20 == 0:
                log(
                    "INFO",
                    "PROCESS TEST",
                    f"Collected {len(positions)} measurements...",
                )
    ...
    log(
        "INFO",
        "PROCESS TEST",
        f"Mean velocity magnitude: {np.mean(np.linalg.norm(velocities, axis=1)):.4f} m/s",
    )
    ...
    log(
        "INFO",
        "PROCESS TEST",
        f"RECOMMENDED process_noise_std (conservative): {estimated_process_noise:.4f}",
    )
    log(
        "INFO",
        "PROCESS TEST",
        f"RECOMMENDED process_noise_std (aggressive): {estimated_process_noise * 2:.4f}",
    )
    log("INFO", "PROCESS TEST", "=" * 60)
    log(
        "INFO",
        "PROCESS TEST",
        "Note: Start with conservative value and increase if filter is too slow",
    )
